...
const BlockedUsersPage = ({ onCloseBlockedContacts }) => {
  const { getContacts, blockedUsers, setSelectedUser } =
  ...
  const filteredBlockedUsers = input
  ...
  return (
    // Stop click propagation so overlay behind modal doesn't close it
    <div
    ...
    >
      {blockedUsers.length === 0 ? (
        <div className="flex items-center justify-center w-full h-20">
          <h1 className="text-lg">No Blocked Contacts</h1>
        </div>
      ) : (
        <div>
          <div className="relative flex items-center justify-between p-4">
            <h1>Blocked Users</h1>
            <button onClick={onCloseBlockedContacts} className="cursor-pointer">
          ...
          <nav className="overflow-y-auto w-full">
            <ul className="flex flex-col">
              {filteredBlockedUsers.map((user, i) => (
                <li
                  key={i}
                  onClick={() => {
                    setSelectedUser(user);
                    onCloseBlockedContacts();
                  }}
